<!-- <h1 align="center">Hi, I'm Resham 👋</h1>

<p align="center">
  <b>AI/ML Engineer • Software Engineer • Final Year CS Undergrad</b><br/>
  Building intelligent systems that solve real problems — from RAG pipelines to adaptive learning tools.
</p>

<p align="center">
  <a href="resham2987@gmail.com">📧 Email</a> •  
  <a href="https://www.linkedin.com/in/resham-3b438a281/">💼 LinkedIn</a> •
  <a href="https://portfolio-three-chi-iwz6yy3ki4.vercel.app/">🌐 Portfolio</a> •
  <a href="https://codolio.com/profile/Resham298">🧩 Coding Profile</a>
</p>

---

## 🚀 About Me

I'm a **B.Tech CSE student at Quantum University** (Graduating 2026, CGPA: 8.3) with hands-on experience building production-grade AI applications, and full-stack web systems.

- 🧠 Passionate about **Generative AI, RAG systems, and LLM applications**
- 🏗️ I build things end-to-end — from embedding pipelines to deployed Projects.
- 🏆 Solved **821+ DSA problems on LeetCode** | Global Rank **46,622 (Top 6%)** | Rating: **1561**
- 🥇 **Top 7 Finalist** at HackIndia Hackathon | **2nd Place** at Genesis Idea Open House 2023
- 🔬 Former **Project Trainee at DRDO** — worked on defense-grade microservices and radar simulation systems

---

## 🛠️ Tech Stack

**Languages**
![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![C++](https://img.shields.io/badge/C++-00599C?style=flat&logo=c%2B%2B&logoColor=white)
![Java](https://img.shields.io/badge/Java-ED8B00?style=flat&logo=openjdk&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-4479A1?style=flat&logo=mysql&logoColor=white)

**AI / ML**
![Machine Learning](https://img.shields.io/badge/Machine%20Learning-FF6F00?style=flat&logo=tensorflow&logoColor=white)
![Generative AI](https://img.shields.io/badge/Generative%20AI-8A2BE2?style=flat&logo=openai&logoColor=white)
![LLMs](https://img.shields.io/badge/LLMs-412991?style=flat&logo=openai&logoColor=white)
![LangChain](https://img.shields.io/badge/LangChain-1C3C3C?style=flat&logo=langchain&logoColor=white)
![RAG](https://img.shields.io/badge/RAG-0052CC?style=flat)
![FAISS](https://img.shields.io/badge/FAISS-0467DF?style=flat)
![Chroma DB](https://img.shields.io/badge/Chroma%20DB-F5A623?style=flat)
![Scikit-learn](https://img.shields.io/badge/Scikit--learn-F7931E?style=flat&logo=scikit-learn&logoColor=white)
![FastAPI](https://img.shields.io/badge/FastAPI-009688?style=flat&logo=fastapi&logoColor=white)
![HuggingFace](https://img.shields.io/badge/HuggingFace-FFD21E?style=flat&logo=huggingface&logoColor=black)

**Web**
![React](https://img.shields.io/badge/React-20232A?style=flat&logo=react&logoColor=61DAFB)
![Next.js](https://img.shields.io/badge/Next.js-000000?style=flat&logo=next.js&logoColor=white)
![Node.js](https://img.shields.io/badge/Node.js-339933?style=flat&logo=node.js&logoColor=white)
![MongoDB](https://img.shields.io/badge/MongoDB-47A248?style=flat&logo=mongodb&logoColor=white)
![TailwindCSS](https://img.shields.io/badge/TailwindCSS-06B6D4?style=flat&logo=tailwindcss&logoColor=white)

**Tools & Platforms**
![Postman](https://img.shields.io/badge/Postman-FF6C37?style=flat&logo=postman&logoColor=white)
![VS Code](https://img.shields.io/badge/VS%20Code-007ACC?style=flat&logo=visual-studio-code&logoColor=white)
![PyCharm](https://img.shields.io/badge/PyCharm-000000?style=flat&logo=pycharm&logoColor=white)
![Google Colab](https://img.shields.io/badge/Google%20Colab-F9AB00?style=flat&logo=googlecolab&logoColor=white)
![Jupyter](https://img.shields.io/badge/Jupyter-F37626?style=flat&logo=jupyter&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=flat&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=flat&logo=github&logoColor=white)


---

## 🔥 Featured Projects

### 🤖 [AskMyPDF AI — PDF RAG Chatbot](https://pdf-rag-chatbot-five.vercel.app)
> `FastAPI` `LangChain` `FAISS` `Llama-3-70B (Groq)` `Cohere Embeddings` `Next.js`

Built a **production-ready multi-document RAG pipeline** that lets users chat with their PDFs and get instant, cited answers.

- 📈 Improved retrieval accuracy from **68% → 84%** after testing on 150 real-world queries
- ⚡ Reduced response latency from **3.1s → 1.5s** via Groq inference, async FastAPI & embedding caching
- 🛡️ Implemented citation grounding + fallback responses to mitigate hallucinations
- 🔧 Pipeline: PDF → 800/150 token chunking → Cohere embeddings → FAISS → MMR retrieval → LLM synthesis

[🔗 Live](https://pdf-rag-chatbot-five.vercel.app) • [📂 GitHub](https://github.com/Resham011/pdf-rag-chatbot) • [🎬 Demo](https://drive.google.com/file/d/1q0ltsKgpwLiPMGQ-PO4k4pBLw-WcibL5/view?usp=drive_link)

---

### 🧠 [AlgoRecall — Intelligent DSA Revision System](#)
> `React.js` `Node.js` `MongoDB` `JWT`

Built an **adaptive spaced-repetition system** to solve inconsistent DSA revision and poor long-term retention during interview prep.

- 📊 Improved revision consistency by **40%** with decay scheduling + difficulty weighting
- 🗂️ **500+ problems logged**, **20+ active users** with personalized revision modes (by company, topic, or randomized)
- 🔐 Secure JWT auth with refresh token rotation; deployed on Vercel with production configs
- 📅 Daily streak tracking, analytics dashboard, dynamic MongoDB schema for fast filtering

[🔗 Live](https://algorecall-vert.vercel.app/) • [📂 GitHub](https://github.com/Resham011/AlgoRecall) • [🎬 Demo](https://drive.google.com/file/d/1SYH13RrisPHmimFCWD8ia2o9Sqo4rStI/view?usp=sharing)

---

### 🎬 [Content-Based Movie Recommendation System](#)
> `Python` `Scikit-learn` `NLP` `TF-IDF` `Streamlit` `TMDB API`

Built a recommendation engine that suggests similar movies based on plot, genres, and cast metadata.

- ⚡ Reduced recommendation lookup time by **60%** via precomputed similarity matrix + NumPy vectorization
- ✅ Validated across **50+ real-world searches** for accurate top suggestions
- 🎨 Deployed interactive Streamlit app with TMDB API-powered poster previews

[🔗 Live](https://movie-recommendation-system-e8pjs6dvu6pcx6ctfhd3hj.streamlit.app/) • [📂 GitHub](https://github.com/Resham011/Movie-Recommendation-System) • [🎬 Demo](https://drive.google.com/file/d/1w8420oeF4mMcytAuJlnyMAW8fH-kO4Z_/view?usp=sharing)

---

## 💼 Work Experience

**🔬 Project Trainee — DRDO (Defence Research and Development Organisation)**
`May 2025 – July 2025 | Delhi, India`
- Engineered **role-based authentication** and secure protocols for defense-grade applications
- Developed **microservices** for real-time radar simulation and high-frequency data visualization
- Optimized geospatial reporting workflows, reducing manual processing time

**🌐 Web Content Intern — GAO Tek Inc.**
`Aug 2024 – Oct 2024 | Remote`
- Optimized **100+ product listings** for SEO using metadata and keyword analytics

---

## 🏆 Achievements

| 🥇 | Achievement |
|----|-------------|
| 💻 | **821+ LeetCode problems** solved — Global Rank 46,622 **(Top 6%)**, Rating: 1561 |
| 🚀 | **Top 7 Finalist** — HackIndia Hackathon |
| 🥈 | **2nd Place** — Genesis Idea Open House 2023 |
| 🎖️ | Participant — **Adobe India Hackathon** |
| 📜 | Certifications: HP LIFE AI Fundamentals, Udemy SQL Bootcamp |

---

## 📊 GitHub Stats

<p align="center">
   <img src="https://github-readme-stats-fast.vercel.app/api/streak?username=Resham011&theme=tokyonight&hide_border=true"/>
  <img src="https://github-readme-stats-fast.vercel.app/api/top-langs/?username=Resham011&layout=compact&theme=tokyonight&hide_border=true&langs_count=8" height="180"/>
</p>

---

<p align="center">
  <i>💻 Always open to collaborating on exciting **AI, ML, based projects**. Let’s innovate together! -->


  <h1 align="center">Resham</h1>

<p align="center">
  <b>AI/ML Engineer &nbsp;·&nbsp; GenAI Developer &nbsp;·&nbsp; Software Engineer</b>
</p>

<p align="center">
  I build production-grade AI systems — RAG pipelines, LLM applications, and intelligent full-stack products that solve real problems.
</p>

<p align="center">
  <a href="mailto:resham2987@gmail.com"><img src="https://img.shields.io/badge/Email-D14836?style=flat&logo=gmail&logoColor=white"/></a>
  <a href="https://www.linkedin.com/in/resham-3b438a281/"><img src="https://img.shields.io/badge/LinkedIn-0077B5?style=flat&logo=linkedin&logoColor=white"/></a>
  <a href="https://portfolio-three-chi-iwz6yy3ki4.vercel.app/"><img src="https://img.shields.io/badge/Portfolio-000000?style=flat&logo=vercel&logoColor=white"/></a>
  <a href="https://codolio.com/profile/Resham298"><img src="https://img.shields.io/badge/LeetCode-FFA116?style=flat&logo=leetcode&logoColor=white"/></a>
</p>

---

## 👤 About Me

- 🎓 **B.Tech CSE** @ Quantum University — CGPA **8.3** | Graduating **2026**
- 🔬 Former **Project Trainee @ DRDO** — defense-grade microservices & radar simulation systems
- 🧠 Specialized in **Generative AI · RAG · NLP · Computer Vision · LLM Applications**
- 💻 **821+ LeetCode problems** | Global Rank **46,622 (Top 6%)** | Rating **1561**
- 🏆 **Top 7 Finalist** — HackIndia Hackathon &nbsp;|&nbsp; **2nd Place** — Genesis Idea Open House 2023
- 🚀 Actively seeking **AI/ML Engineer / GenAI Developer** roles (Full-time & Internship)

---

## 🛠️ Tech Stack

**Programming**

![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![C++](https://img.shields.io/badge/C++-00599C?style=flat&logo=c%2B%2B&logoColor=white)
![Java](https://img.shields.io/badge/Java-ED8B00?style=flat&logo=openjdk&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-4479A1?style=flat&logo=mysql&logoColor=white)

**ML / DL Frameworks**

![PyTorch](https://img.shields.io/badge/PyTorch-EE4C2C?style=flat&logo=pytorch&logoColor=white)
![Scikit-learn](https://img.shields.io/badge/Scikit--learn-F7931E?style=flat&logo=scikit-learn&logoColor=white)
![HuggingFace](https://img.shields.io/badge/HuggingFace-FFD21E?style=flat&logo=huggingface&logoColor=black)
![Computer Vision](https://img.shields.io/badge/Computer%20Vision-5C3EE8?style=flat&logo=opencv&logoColor=white)

**GenAI / LLM**

![LangChain](https://img.shields.io/badge/LangChain-1C3C3C?style=flat&logo=langchain&logoColor=white)
![RAG](https://img.shields.io/badge/RAG-0052CC?style=flat)
![LLMs](https://img.shields.io/badge/LLMs-412991?style=flat&logo=openai&logoColor=white)
![FAISS](https://img.shields.io/badge/FAISS-0467DF?style=flat)
![Chroma DB](https://img.shields.io/badge/Chroma%20DB-F5A623?style=flat)
![Groq](https://img.shields.io/badge/Groq-FF4F00?style=flat)

**Data Tools**

![Pandas](https://img.shields.io/badge/Pandas-150458?style=flat&logo=pandas&logoColor=white)
![NumPy](https://img.shields.io/badge/NumPy-013243?style=flat&logo=numpy&logoColor=white)
![Jupyter](https://img.shields.io/badge/Jupyter-F37626?style=flat&logo=jupyter&logoColor=white)

**Dev & Web**

![FastAPI](https://img.shields.io/badge/FastAPI-009688?style=flat&logo=fastapi&logoColor=white)
![React](https://img.shields.io/badge/React-20232A?style=flat&logo=react&logoColor=61DAFB)
![Next.js](https://img.shields.io/badge/Next.js-000000?style=flat&logo=next.js&logoColor=white)
![Node.js](https://img.shields.io/badge/Node.js-339933?style=flat&logo=node.js&logoColor=white)
![MongoDB](https://img.shields.io/badge/MongoDB-47A248?style=flat&logo=mongodb&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat&logo=docker&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=flat&logo=git&logoColor=white)
![TailwindCSS](https://img.shields.io/badge/TailwindCSS-06B6D4?style=flat&logo=tailwindcss&logoColor=white)

---

## 🚀 Featured Projects

### 🤖 [AskMyPDF — Production RAG Chatbot](https://pdf-rag-chatbot-five.vercel.app)

> **Problem:** Extracting accurate, cited answers from multiple PDFs at scale with low latency.

`FastAPI` `LangChain` `FAISS` `Llama-3-70B (Groq)` `Cohere Embeddings` `Next.js`

- 📈 Retrieval accuracy: **68% → 84%** (validated on 150 real-world queries)
- ⚡ Latency: **3.1s → 1.5s** via Groq inference + async FastAPI + embedding caching
- 🛡️ Citation grounding + fallback responses to mitigate hallucinations
- 🔧 Pipeline: PDF ingestion → 800/150-token chunking → embeddings → FAISS → MMR retrieval → LLM synthesis

[🔗 Live](https://pdf-rag-chatbot-five.vercel.app) &nbsp;|&nbsp; [📂 GitHub](https://github.com/Resham011/pdf-rag-chatbot) &nbsp;|&nbsp; [🎬 Demo](https://drive.google.com/file/d/1q0ltsKgpwLiPMGQ-PO4k4pBLw-WcibL5/view?usp=drive_link)

---

### 👗 [Hybrid Fashion Recommendation System](https://huggingface.co/spaces/Resham2987/Abaya-and-Thobes-Recommendation-System)

> **Problem:** Single-model recommenders fail to capture visual + behavioral user preferences together.

`PyTorch` `MobileNetV2` `FastAPI` `Scikit-learn`

- 🎯 **88% validation accuracy** combining collaborative filtering, metadata similarity, and visual feature extraction
- 📷 Applied **Computer Vision** (MobileNetV2) for image-based item matching
- 🏗️ Modular, testable backend API architecture — scalable to production

---

### 🧠 [AlgoRecall — Adaptive DSA Revision System](https://algorecall-vert.vercel.app/)

> **Problem:** Inconsistent revision and poor long-term retention of DSA concepts during interview prep.

`React.js` `Node.js` `MongoDB` `JWT`

- 📊 Revision consistency improved by **40%** via spaced-repetition + decay scheduling
- 🗂️ 500+ problems logged · 20+ active users · personalized modes (by company, topic, random)
- 🔐 JWT auth with refresh token rotation · daily streak tracking · analytics dashboard

[🔗 Live](https://algorecall-vert.vercel.app/) &nbsp;|&nbsp; [📂 GitHub](https://github.com/Resham011/AlgoRecall) &nbsp;|&nbsp; [🎬 Demo](https://drive.google.com/file/d/1SYH13RrisPHmimFCWD8ia2o9Sqo4rStI/view?usp=sharing)

---

### 🎬 [Movie Recommendation System](https://movie-recommendation-system-e8pjs6dvu6pcx6ctfhd3hj.streamlit.app/)

> **Problem:** Discovering similar movies based on plot, genre, and cast — fast and accurately.

`Python` `Scikit-learn` `TF-IDF` `NLP` `Streamlit` `TMDB API`

- ⚡ **60% faster** lookups via precomputed similarity matrix + NumPy vectorization
- ✅ Validated across 50+ real-world searches
- 🎨 Interactive Streamlit app with live TMDB poster previews

[🔗 Live](https://movie-recommendation-system-e8pjs6dvu6pcx6ctfhd3hj.streamlit.app/) &nbsp;|&nbsp; [📂 GitHub](https://github.com/Resham011/Movie-Recommendation-System)

---

## 💼 Experience

**🔬 Project Trainee — DRDO (Defence Research and Development Organisation)**
`May 2025 – July 2025 · Delhi, India`
- Built **role-based authentication** and secure protocols for defense-grade systems
- Developed **microservices** for real-time radar simulation and high-frequency data visualization
- Optimized geospatial reporting workflows, reducing manual processing time

**🌐 Web Content Intern — GAO Tek Inc.**
`Aug 2024 – Oct 2024 · Remote`
- Optimized **100+ product listings** for SEO using metadata and keyword analytics

---

## 🏆 Achievements

| | |
|---|---|
| 💻 | **821+ LeetCode problems** · Global Rank **46,622 (Top 6%)** · Rating **1561** |
| 🚀 | **Top 7 Finalist** — HackIndia Hackathon |
| 🥈 | **2nd Place** — Genesis Idea Open House 2023 (Startup) |
| 🎖️ | Participant — Adobe India Hackathon |
| 📜 | HP LIFE AI Fundamentals · Udemy SQL Bootcamp |

---

## 📊 GitHub Stats

<p align="center">
   <img src="https://github-readme-stats-fast.vercel.app/api/streak?username=Resham011&theme=tokyonight&hide_border=true"/>
  <img src="https://github-readme-stats-fast.vercel.app/api/top-langs/?username=Resham011&layout=compact&theme=tokyonight&hide_border=true&langs_count=8" height="180"/>
</p>

---

<p align="center">
  <i>Open to AI/ML, GenAI, and Software Engineering roles · Full-time & Internship · 2026</i>
</p>
